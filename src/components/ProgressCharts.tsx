...
export default function ProgressCharts({
  ...
  const fetchWeeklyStats = async () => {
    try {
      ...
      const {
        data,
        error
      } = await supabase.from('daily_stats').select('*').eq('user_id', userId).gte('date', sevenDaysAgo.toISOString().split('T')[0]).order('date', {
        ascending: true
      });
  ...
  return <div className={`space-y-4 ${compact ? 'animate-fade-slide-up' : ''}`}>
        ...
        <CardContent>
          <ResponsiveContainer width="100%" height={compact ? 200 : 300}>
            <LineChart data={weeklyData}>
              ...
              <Legend />
              <Line type="monotone" dataKey="tasks_completed" stroke={COLORS.tasks} strokeWidth={2} name="Tasks" dot={{
              fill: COLORS.tasks
            }} />
              <Line type="monotone" dataKey="videos_watched" stroke={COLORS.videos} strokeWidth={2} name="Videos" dot={{
              fill: COLORS.videos
            }} />
              <Line type="monotone" dataKey="materials_read" stroke={COLORS.materials} strokeWidth={2} name="Materials" dot={{
              fill: COLORS.materials
            }} />
            </LineChart>
          </ResponsiveContainer>
      ...
      {!compact && <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
