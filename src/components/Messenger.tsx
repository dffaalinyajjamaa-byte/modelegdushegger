...
import { Send, Search, Plus, Image as ImageIcon, Paperclip, Mic, MicOff, MoreVertical, Check, CheckCheck, Users, UserPlus, AlertCircle, Ban, Pin, X, ArrowLeft, Reply, MessageSquare, SmilePlus, Loader2, UserCheck, Clock, Heart } from 'lucide-react';
...
export default function Messenger({ user, onBack }: MessengerProps) {
  ...
  const [recommendedFriends, setRecommendedFriends] = useState<MessagingUser[]>([]);
  const [userBadges, setUserBadges] = useState<Record<string, UserBadgeInfo>>({});
  ...
  const { typingUsers, handleTyping, stopTyping } = useTypingIndicator(selectedChat?.chat_id || null, user?.id || '');
  ...
  const fetchRecommendedFriends = async () => {
    ...
    const myPoints = myRanking?.total_points || 0;

    // Get users with similar scores (within 200 points)
    const { data: rankings } = await supabase
      .from('user_rankings')
      .select('user_id, total_points')
      .neq('user_id', user.id)
      .gte('total_points', myPoints - 200)
      .lte('total_points', myPoints + 200)
    ...
    if (rankings && rankings.length > 0) {
      ...
      if (msgUsers) {
        ...
        const recommended = msgUsers.filter(u => !acceptedFriends.includes(u.user_id));
        setRecommendedFriends(recommended.slice(0, 5));
      }
    }
  };

  const fetchUserBadges = async (userIds: string[]) => {
    // Fetch teacher roles
    ...
    const verifiedIds = new Set(profiles?.filter(p => p.is_verified)?.map(p => p.user_id) || []);

    // Fetch rankings for top students
    const { data: rankings } = await supabase
      .from('user_rankings')
      .select('user_id, total_points')
      .in('user_id', userIds)
    ...
    // Get top 3 non-teacher user IDs
    const topStudentIds = rankings
      ?.filter(r => !teacherIds.has(r.user_id))
      .slice(0, 3)
      .map(r => r.user_id) || [];

    const badges: Record<string, UserBadgeInfo> = {};
    userIds.forEach(id => {
      badges[id] = {
        isTeacher: teacherIds.has(id),
        isVerified: verifiedIds.has(id),
        isTopStudent: topStudentIds.includes(id),
        totalPoints: rankings?.find(r => r.user_id === id)?.total_points || 0
      };
    });
  ...
  const initializeMessaging = async () => {
    ...
    const { data: messagingUser } = await supabase.from('messaging_users').select('*').eq('user_id', user.id).single();
    ...
    // Fetch all users
    const { data: allUsers } = await supabase.from('messaging_users').select('*').neq('user_id', user.id);
    if (allUsers) {
      setUsers(allUsers);
      // Fetch badges for all users
      fetchUserBadges(allUsers.map(u => u.user_id));
    }
  };

  const fetchChats = async () => {
    const { data } = await supabase.from('chats').select('*').contains('members', [user.id]).order('updated_at', { ascending: false });
    if (data) setChats(data);
  };

  const fetchMessages = async () => {
    if (!selectedChat) return;
    const { data } = await supabase.from('messages').select('*').eq('chat_id', selectedChat.chat_id).order('timestamp', { ascending: true });
  ...
  const searchUsers = async () => {
    const { data } = await supabase.from('messaging_users').select('*').or(`name.ilike.%${searchQuery}%,search_id.ilike.%${searchQuery}%`).neq('user_id', user.id);
    setUsers(data || []);
  };
  ...
  const searchUsersByNameOrId = async (query: string) => {
    ...
    try {
      const { data, error } = await supabase.rpc('search_users_similar', {
        search_term: query,
      ...
      if (!error && data) {
        ...
        if (data.length > 0) {
          fetchUserBadges(data.map((u: MessagingUser) => u.user_id));
        }
        return;
      }
    } catch (e) {
    ...
    const { data } = await supabase.from('messaging_users').select('*').or(`name.ilike.%${query}%,search_id.ilike.%${query}%,user_id.ilike.%${query}%`).neq('user_id', user.id).limit(20);
    setUserSearchResults(data || []);
  ...
  const canChatWith = (otherUserId: string) => {
    // Teachers can chat with anyone
    if (isTeacher) return true;
    // Students can only chat with accepted friends
    return acceptedFriends.includes(otherUserId);
  };

  const createOrSelectChat = async (otherUser: MessagingUser) => {
    // Check if they can chat
    if (!canChatWith(otherUser.user_id)) {
      toast({
        title: "Cannot start chat",
        description: "You need to be friends first. Send a friend request!",
        variant: "destructive"
      });
      return;
    }
    ...
    const { data: existingChat } = await supabase.from('chats').select('*').eq('chat_id', chatId).single();
  ...
  const renderUserBadge = (userId: string) => {
    const badge = userBadges[userId];
    if (!badge) return null;
    
    if (badge.isTeacher) {
      return <VerifiedBadge type="gold" size="sm" />;
    }
    if (badge.isTopStudent) {
  ...
  // Filter chats to only show friends (unless teacher)
  const filteredChats = isTeacher 
    ? chats 
    : chats.filter(chat => {
        const otherUserId = chat.members.find(m => m !== user.id);
        return otherUserId && acceptedFriends.includes(otherUserId);
      });
            ...
            <div className="flex-1 overflow-hidden">
              ...
              {activeTab === 'friends' && (
                <ScrollArea className="h-full p-4">
                  <h3 className="text-sm font-semibold text-muted-foreground mb-3">Your Friends ({acceptedFriends.length})</h3>
                  {acceptedFriends.length === 0 ? (
                    <div className="text-center text-muted-foreground py-8">
                      <Users className="w-12 h-12 mx-auto mb-4 opacity-50" />
                      <p>No friends yet</p>
                      <p className="text-sm mt-1">Search for users and send friend requests!</p>
                    </div>
                  ) : (
                    <div className="space-y-2">
                      {users.filter(u => acceptedFriends.includes(u.user_id)).map(friend => (
                        <Card 
                          key={friend.id} 
                          className="cursor-pointer hover:bg-accent/50 transition-colors"
                          onClick={() => createOrSelectChat(friend)}
                        >
                          <CardContent className="p-3 flex items-center gap-3">
                            <div className="relative">
                              <ChatBubbleAvatar src={friend.profile_pic || ''} fallback={friend.name.charAt(0)} />
                              <div className="absolute -bottom-0.5 -right-0.5 w-3 h-3 rounded-full bg-green-500 ring-2 ring-background" />
                            </div>
                            <div className="flex-1 min-w-0">
                              <h3 className="font-semibold text-sm truncate flex items-center gap-1.5">
                                {friend.name}
                                {renderUserBadge(friend.user_id)}
                              </h3>
            ...
            {selectedChat ? (
                  ...
                  <div className="flex items-center gap-3 flex-1 min-w-0">
                    ...
                    <div className="flex-1 min-w-0">
                      <h3 className="font-semibold text-foreground truncate flex items-center gap-1.5">
                        ...
                        {getChatUser(selectedChat) && renderUserBadge(getChatUser(selectedChat)!.user_id)}
                      </h3>
                      <p className="text-xs text-muted-foreground">Online</p>
                    </div>
                  </div>
                          ...
                          <div key={message.id} className={`flex items-start gap-2 ${isOwnMessage ? 'flex-row-reverse' : ''}`}>
                            ...
                            <div className={`flex flex-col gap-1 max-w-[75%] ${isOwnMessage ? 'items-end' : 'items-start'}`}>
                              <div className={`rounded-2xl px-4 py-2 ${isOwnMessage ? 'bg-primary text-primary-foreground' : 'bg-muted text-foreground'}`}>
                                ...
                                {repliedMessage && (
                                  <div className={`mb-2 pb-2 border-l-2 pl-2 rounded text-xs ${isOwnMessage ? 'border-primary-foreground/30 bg-primary-foreground/10' : 'border-primary/30 bg-muted-foreground/10'}`}>
                              ...
                              <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                ...
                                {replyCount > 0 && (
                                  <Button variant="ghost" size="sm" className="h-6 text-xs px-2" onClick={() => handleViewThread(message)}>
                ...
                <div className="p-4 pb-24 md:pb-4 border-t border-border/50 bg-background/95 backdrop-blur-sm safe-area-bottom">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <Button 
                      ...
                      className={`flex-shrink-0 h-10 w-10 rounded-full transition-all ${isListening ? 'animate-pulse ring-2 ring-destructive ring-offset-2' : ''}`}
                    >
              ...
              <div className="flex-1 items-center justify-center bg-background flex flex-col">
                <div className="text-center text-muted-foreground">
                  ...
                  <p className="text-sm mt-1">Or find friends to connect with</p>
                </div>
              </div>
            )}
          </div>
              ...
              <ScrollArea className="h-72">
                <div className="space-y-2">
                  {userSearchResults.length === 0 && userSearchQuery ? (
                    <div className="text-center text-muted-foreground py-8">
                      <p>No users found</p>
                      <p className="text-xs mt-1">Try searching by name</p>
                    </div>
                  ) : (
                    userSearchResults.map(u => {
                      const isFriend = acceptedFriends.includes(u.user_id);
                      const isPending = sentRequests.includes(u.user_id);
                      
                      return (
                        <Card key={u.id} className="cursor-pointer hover:bg-accent/50 transition-colors">
                          <CardContent className="p-3 flex items-center gap-3">
                            <ChatBubbleAvatar src={u.profile_pic || ''} fallback={u.name[0]} />
                            <div className="flex-1 min-w-0">
                              <h3 className="font-semibold text-sm truncate flex items-center gap-1.5">
                                {highlightMatch(u.name, userSearchQuery)}
                                {renderUserBadge(u.user_id)}
                              </h3>
                              <p className="text-xs text-muted-foreground">
                                {isFriend ? 'Friend' : isPending ? 'Request sent' : 'Not connected'}
                              </p>
                            </div>
                            {isFriend ? (
                              <Button 
          ...
          <DialogContent className="sm:max-w-md">
            ...
            <div className="space-y-4">
              ...
              <ScrollArea className="h-64">
                <div className="space-y-2">
                  {users.filter(u => acceptedFriends.includes(u.user_id)).length === 0 ? (
                    <div className="text-center text-muted-foreground py-8">
                      <p>No friends yet</p>
                      <p className="text-sm mt-1">Search for users and send friend requests first!</p>
                      <Button 
                        className="mt-4" 
                        onClick={() => {
                          setIsFindUserDialogOpen(false);
                    ...
                    users.filter(u => acceptedFriends.includes(u.user_id)).map(u => (
                      <Card 
                        key={u.id} 
                        className="cursor-pointer hover:bg-accent/50 transition-colors" 
