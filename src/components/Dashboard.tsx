...
export default function Dashboard({
  ...
  const handleContentClick = (item: Content) => {
    ...
    if (item.type === 'video') {
      setActiveView('video');
      logActivity('video', `Started watching: ${item.title}`, {
        content_id: item.id
      });
    } else if (item.type === 'pdf') {
      setActiveView('pdf');
      logActivity('pdf', `Opened document: ${item.title}`, {
        content_id: item.id
      });
    }
  };
  const renderActiveView = () => {
    switch (activeView) {
      ...
      case 'tasks':
        return <TaskManager user={user} onLogActivity={logActivity} onTaskComplete={() => updateUserStats('tasks_completed')} />;
      case 'settings':
      ...
      case 'books':
        return <Suspense fallback={<LoadingFallback />}>
            <DigitalBooksLibrary user={user} onBack={() => setActiveView('dashboard')} onBookClick={handleContentClick} />
          </Suspense>;
      case 'videos':
        return <Suspense fallback={<LoadingFallback />}>
            <VideoLessonsLibrary user={user} onBack={() => setActiveView('dashboard')} onVideoClick={handleContentClick} />
          </Suspense>;
      case 'video':
        return selectedContent ? <VideoViewer content={selectedContent} user={user} onBack={() => setActiveView('dashboard')} onLogActivity={logActivity} onVideoWatched={() => updateUserStats('videos_watched')} /> : null;
      case 'pdf':
        return selectedContent ? <PDFViewer content={selectedContent} user={user} onBack={() => setActiveView('dashboard')} onLogActivity={logActivity} onMaterialRead={() => updateUserStats('materials_read')} /> : null;
      default:
        return renderDashboard();
    }
  };
  const renderDashboard = () => <div className="space-y-6 sm:space-y-8">
      ...
      <section className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-primary via-primary/90 to-accent p-6 text-primary-foreground shadow-glow">
        <div className="relative z-10 flex flex-col items-center text-center">
          <img src={logo} alt="Model Egdu" className="w-20 h-20 mb-3 rounded-full shadow-glow" />
      ...
      {!streak.loading && <StreakBadge currentStreak={streak.streak.current} longestStreak={streak.streak.longest} totalPoints={userRanking?.total_points || 0} className="shadow-lg" />}
      ...
      <DashboardTabs activeTab={activeTab} onTabChange={setActiveTab}>
        ...
        <TabsContent value="all" className="space-y-6">
          ...
          <RecentLessons userId={user.id} onLessonClick={(content, playbackTime) => {
          setSelectedContent({
            id: content.id,
            title: content.title,
        ...
        <TabsContent value="lessons" className="space-y-6">
          <div className="grid lg:grid-cols-2 gap-6">
            <div>
              ...
              <VideoLessonsLibrary user={user} onBack={() => {}} onVideoClick={handleContentClick} embedded={true} />
            </div>
            <div>
              <h2 className="text-xl font-bold mb-4">Digital Books</h2>
              <DigitalBooksLibrary user={user} onBack={() => {}} onBookClick={handleContentClick} embedded={true} />
            </div>
          </div>
        </TabsContent>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <button onClick={() => setActiveView('ai-teacher')} className="group relative quick-access-card bg-gradient-to-br from-blue-500/20 to-purple-500/20 border-2 border-blue-500/30 hover:border-blue-500/60 p-8 rounded-2xl transition-all duration-300 hover:scale-110 hover:shadow-glow overflow-hidden">
        ...
        <button onClick={() => setActiveView('tasks')} className="group relative quick-access-card bg-gradient-to-br from-green-500/20 to-teal-500/20 border-2 border-green-500/30 hover:border-green-500/60 p-8 rounded-2xl transition-all duration-300 hover:scale-110 hover:shadow-glow overflow-hidden">
        ...
        <button onClick={() => setActiveView('relax-time')} className="group relative quick-access-card bg-gradient-to-br from-purple-500/20 to-violet-500/20 border-2 border-purple-500/30 hover:border-purple-500/60 p-8 rounded-2xl transition-all duration-300 hover:scale-110 hover:shadow-glow overflow-hidden">
        ...
        <button onClick={() => setActiveView('national-exams')} className="group relative quick-access-card bg-gradient-to-br from-rose-500/20 to-red-500/20 border-2 border-rose-500/30 hover:border-rose-500/60 p-8 rounded-2xl transition-all duration-300 hover:scale-110 hover:shadow-glow overflow-hidden">
        ...
        <button onClick={() => setActiveView('science-experiments')} className="group relative quick-access-card bg-gradient-to-br from-cyan-500/20 to-teal-500/20 border-2 border-cyan-500/30 hover:border-cyan-500/60 p-8 rounded-2xl transition-all duration-300 hover:scale-110 hover:shadow-glow overflow-hidden">
        ...
        <button onClick={() => setActiveView('messenger')} className="group relative quick-access-card bg-gradient-to-br from-pink-500/20 to-rose-500/20 border-2 border-pink-500/30 hover:border-pink-500/60 p-8 rounded-2xl transition-all duration-300 hover:scale-110 hover:shadow-glow overflow-hidden">
        ...
        <button onClick={() => setActiveView('quiz')} className="group relative quick-access-card bg-gradient-to-br from-orange-500/20 to-yellow-500/20 border-2 border-orange-500/30 hover:border-orange-500/60 p-8 rounded-2xl transition-all duration-300 hover:scale-110 hover:shadow-glow overflow-hidden">
        ...
        <button onClick={() => setActiveView('competition')} className="group relative quick-access-card bg-gradient-to-br from-yellow-500/20 to-amber-500/20 border-2 border-yellow-500/30 hover:border-yellow-500/60 p-8 rounded-2xl transition-all duration-300 hover:scale-110 hover:shadow-glow overflow-hidden">
        ...
        <button onClick={() => setActiveView('worksheets')} className="group relative quick-access-card bg-gradient-to-br from-indigo-500/20 to-violet-500/20 border-2 border-indigo-500/30 hover:border-indigo-500/60 p-8 rounded-2xl transition-all duration-300 hover:scale-110 hover:shadow-glow overflow-hidden">
  ...
  return <div className="min-h-screen bg-background pb-20 md:pb-6">
      ...
      <header className="sticky top-0 z-50 bg-background/95 backdrop-blur-lg border-b border-border/50 shadow-sm">
        <div className="container mx-auto px-4 py-4 flex items-center justify-between">
          {/* Left: Back Button */}
          {activeView !== 'dashboard' && <Button variant="ghost" size="sm" onClick={() => setActiveView('dashboard')} className="gap-2 hover:bg-muted">
