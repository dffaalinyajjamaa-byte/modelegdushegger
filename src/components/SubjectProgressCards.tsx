...
const SubjectProgressCards = ({
  ...
  const fetchSubjectProgress = async () => {
    try {
      const {
        data,
        error
      } = await supabase.from('user_activity_log').select('subject, duration_minutes').eq('user_id', userId).not('subject', 'is', null);
      ...
      const progressData: SubjectProgress[] = Array.from(subjectMap.entries()).map(([subject, duration]) => {
      ...
      }).sort((a, b) => b.duration_minutes - a.duration_minutes).slice(0, 8);
  ...
  if (loading) {
    return <div className="subject-grid">
        {[1, 2, 3, 4].map(i => <Card key={i} className="subject-card subject-card-blue animate-pulse h-40" />)}
      </div>;
  }
  ...
  return <div className="subject-grid">
      {subjects.map(subject => <Card key={subject.subject} className={`subject-card subject-card-${subject.color} hover-scale cursor-pointer`}>
